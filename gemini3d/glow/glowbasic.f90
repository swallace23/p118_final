...
integer :: u
character(1024) :: config_filename

namelist /input/ idate,ut,glat,glong,f107a,f107,f107p,ap,ef,ec
...
!
! Get input values:
if (command_argument_count() < 1) error stop 'please tell me what .nml input to use'
call get_command_argument(1, config_filename)
open(newunit=u, file=config_filename, status='old', action='read')
read(u,nml=input)
close(u)
...
! Output section:
!
write(stdout,"(1x,i7,9f8.1)") idate,ut,glat,glong,f107a,f107,f107p,ap,ef,ec
write(stdout,"(A)") '   Z     Tn       O        N2        NO      Ne(in)    Ne(out)  Ionrate' &
...
write(stdout,"(A)") '   Z      3371    4278    5200    5577    6300    7320   10400    3644    7774' &
                  //'    8446    3726    LBH     1356    1493    1304'
